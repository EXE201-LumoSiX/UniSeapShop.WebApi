...
public class CartService : ICartService
{
    ...
    public async Task<CartDto> UpdateItemQuantityAsync(UpdateCartItemDto updateItemDto)
    {
        ...
        var cartItem =
            await _unitOfWork.CartItems.FirstOrDefaultAsync(ci =>
                ci.CartId == cart.Id && ci.ProductId == updateItemDto.ProductId
            );
    ...
    public async Task<CartDto> RemoveItemFromCartAsync(Guid productId)
    {
        ...
        var cartItem =
            await _unitOfWork.CartItems.FirstOrDefaultAsync(ci => ci.CartId == cart.Id && ci.ProductId == productId
            );
        ...
        Dictionary<Guid, Product>? productDict = null)
    {
        ...
        if (cartItems == null) cartItems = await _unitOfWork.CartItems.GetAllAsync(ci => ci.CartId == cart.Id);
